...
const listFilesFromShareUrl = async (shareUrl) => {
  try {
    ...
    while ((match = filePattern.exec(html)) !== null) {
      const [_, id, name, mimeType] = match;
...
const downloadFile = async (fileId) => {
  try {
    ...
    // 非同期でダウンロード開始
    downloadFileAsync(fileId, localPath, downloadInfo);
...
const getDownloadStatus = async (fileId) => {
  try {
    ...
    const activeDownload = Object.values(activeDownloads)
      .find(d => d.fileId === fileId);
...
const streamFile = async (streamData) => {
  try {
    ...
    // ストリーミング開始
    return await streamService.startStream({
      ...streamData,
