...
describe('FileService', () => {
  ...
  describe('saveFileInfo', () => {
    ...
    it('should handle write error', async () => {
      ...
      await expect(fileService.saveFileInfo(fileData)).rejects.toThrow(
        'ファイル情報の保存に失敗しました'
      );
    });
  });
  ...
  describe('deleteFile', () => {
    ...
    it('should throw error for non-existent file', async () => {
      await expect(fileService.deleteFile('999')).rejects.toThrow('ファイルが見つかりません');
    });
  ...
  describe('updateFileStatus', () => {
    ...
    it('should throw error for non-existent file', async () => {
      await expect(fileService.updateFileStatus('999', 'processing')).rejects.toThrow(
        'ファイルが見つかりません'
      );
    });
    ...
    it('should handle write error', async () => {
      ...
      await expect(fileService.updateFileStatus('1', 'processing')).rejects.toThrow(
        'ファイル状態の更新に失敗しました'
      );
    });
  });
