...
# Start essential services only (rtmp-server + controller)
log_message "Starting essential services..."
sudo -u $APP_USER docker-compose -f $COMPOSE_FILE --env-file .env.production up -d rtmp-server controller
...
ExecStart=/usr/local/bin/docker-compose -f $COMPOSE_FILE --env-file .env.production up -d rtmp-server controller
ExecStop=/usr/local/bin/docker-compose -f $COMPOSE_FILE --env-file .env.production down
ExecReload=/usr/local/bin/docker-compose -f $COMPOSE_FILE --env-file .env.production restart
TimeoutStartSec=600
TimeoutStopSec=120
Restart=on-failure
RestartSec=30
...
if docker-compose -f $COMPOSE_FILE ps | grep -q "Up"; then
    ...
    log_message "WARNING: Services may not be fully started yet. Check logs with: docker-compose logs"
    docker-compose -f $COMPOSE_FILE ps >> $LOG_FILE
fi
